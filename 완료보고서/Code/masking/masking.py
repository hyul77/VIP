...
#CUDA_VISIBLE_DEVICES=0 python masking.py --model saves/s03_pcvos_ytvos.pth --output ./youtube_val_2019/s03_pcvos_ytvos_c5/ --clip_length 5 --refine_clip ICR --T_window 2 --S_window 7 --shared_proj --memory_read PMM --predict_all --time


#CUDA_VISIBLE_DEVICES=0 python ./masking/masking.py --model saves/s03_pcvos_ytvos.pth --output ./youtube_val_2019/s03_pcvos_ytvos_c5/ --clip_length 5 --refine_clip ICR --T_window 2 --S_window 7 --shared_proj --memory_read PMM --predict_all --time


#CUDA_VISIBLE_DEVICES=0 python ./masking/masking.py --clip_length 5 --refine_clip ICR --T_window 2 --S_window 7 --shared_proj --memory_read PMM --predict_all --time
...
parser.add_argument('--model', default='/home/hyul/Desktop/ViVE/masking/saves/s03_pcvos_ytvos.pth')
parser.add_argument('--yv_path', default='/home/hyul/Desktop/ViVE/inputdata')
...
parser.add_argument('--output', default = '/home/hyul/Desktop/ViVE/inputdata')
...
palette = Image.open(path.expanduser(yv_path + '/Annotations/video/00000.png')).getpalette()
...
#for data in progressbar(test_loader):
    with torch.cuda.amp.autocast(enabled=args.amp):
        ...
        if not args.output_all:
            ...
            start_req_frames_names = req_frames_names
            ...
            '''if (start_req_frames_names != '00000'):
                req_frames = sorted(req_frames, reverse=True)
                for fi in range(rgb.shape[1]):
                    frame_name = info['frames'][fi][0][:-4]
                if frame_name in req_frames_names:
                    req_frames.append(fi)
        ...
        # Save the results
        this_out_path = path.join(out_path, 'Annotation', name)
        ...
        for f in range(idx_masks.shape[0]):
            if f >= min_idx:
                if args.output_all or (f in req_frames):
                    ...
                    img_E.save(os.path.join(this_out_path, info['frames'][f][0].replace('.jpg','.png'))) #여기 문제인거같은데
